    ...
    def move(self,board):
        ...
        if self.position+moveroll <=99:
            ...
            if self.player_num == 1:
                p1p[oldpos] = EMPTY
                p1p[self.postition] = self.peice

            elif self.player_num == 2:
                p2p[oldpos] = EMPTY
                p2p[self.postition] = self.peice

            elif self.player_num == 3:
                p3p[oldpos] = EMPTY
                p3p[self.postition] = self.peice

            elif self.player_num == 4:
                p4p[oldpos] = EMPTY
                p4p[self.postition] = self.peice
            else:
                pass
...
def ask_num():
    x = True
    while x == True:
        num = input("how many players are there? ")
        num_players = int(num)
        if num_players <= 5:
            print("alright ",num_players, " players it is")
...
def main():
    ...
    board.create_spaces()
    while not winner:
        print(players[turn].name +" peice"+players[turn].peice+" its your turn")
